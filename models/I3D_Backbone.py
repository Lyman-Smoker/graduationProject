...
class I3D_backbone(nn.Module):
    ...
    def forward(self, x_1, x_2, is_train=None, label=None, theta=None):
        # spatiotemporal feature
        total_video = torch.cat((x_1, x_2), 0)  # 2N C H W
        ...
        # video_pack = torch.cat([x_1[:, :, i: i + 16] for i in start_idx])  # 10*2N, c, 16, h, w
        # total_feature = self.backbone(video_pack).reshape(10, len(x_1), -1).transpose(0, 1)  # 2N * 10 * 1024
...
if __name__ == '__main__':
    ...
    input1 = torch.randn((4, 3, 103, 224, 224))
    input2 = torch.randn((4, 3, 103, 224, 224))
    out = base_model(input1, input2)
    print(out[0].shape)
