...
class Bidir_Attention(nn.Module):
    def __init__(self, dim=1024, mask=False, ln_mlp=False, dropout=0.6):
    ...
    def forward(self, feature1, feature2, topk=4):
        ...
        if self.ln_mlp:
            feature_2to1_norm = self.ln(feature_2to1)
            feature_2to1 = feature_2to1 + self.mlp(feature_2to1_norm)
            feature_1to2_norm = self.ln(feature_1to2)
            feature_1to2 = feature_1to2 + self.mlp(feature_1to2_norm)
